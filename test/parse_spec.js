...
describe("parse", function() {
  ...
  it("can parse scientific notation with a float coeffecient", function() {
    var fn = parse('.42e2');
    expect(fn()).toBe(42);
  });
  ...
  it("can parse a string in double qoutes", function() {
    var fn = parse('"abc"');
    expect(fn()).toBe('abc');
  });

  it("will not parse a string with mismathching quotes", function() {
  ...
  it("uses locals instead of scope when the first par matches", function() {
    var fn = parse('aKey.anotherKey');
  ...
  it("parses a simple computed numeric array access", function() {
  ...
  it("parses multiple OR", function() {
    expect(parse('true || true || true')()).toBe(true);
